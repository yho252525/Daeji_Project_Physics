...
float aA, mA=10, vA=4, mvA=40; //physical status of box A
float aB, mB=10, xvB=4, yvB=0, mvB=40; //physical status of box B
...
void ManagePhysics() {
  if (boxA.y - mA*100/30<=boxB.y) {  //if collided
  ...
  if (collision) { //after collision
    ...
    yvB += 9.8/frameRate;  //accelerating
  }
}
